...
import { GoogleUserDto, CreateTokenDto, AuthResponseDto } from './dto/auth.dto';
...
export class AuthController {
  ...
  @Post('token')
  @ApiOperation({ 
    summary: 'Crear nuevo token de acceso',
    description: 'Crea un nuevo token de acceso para un usuario autenticado'
  })
  @ApiResponse({ status: 201, description: 'Token creado exitosamente', type: AuthResponseDto })
  async createToken(@Body() createTokenDto: CreateTokenDto): Promise<{ token: string }> {
    const token = await this.authService.createToken(createTokenDto.userId, createTokenDto.name);
    return { token };
  }
